...
class adjustedCell(
    ...
    var xGridArray:ArrayList<Double> = Matrix_Cell.gridX(xArray, 0.01, xArray.max()),
    var yGridArray: ArrayList<Double> = Matrix_Cell.gridY(yArray, 0.01, yArray.max()),
...

{
    fun changeParams(name: String, illu:Double, temp:Double){
        ...
        xGridArray = Matrix_Cell.gridX(xArray, 0.01, xArray.max())
        yGridArray = Matrix_Cell.gridY(yArray, 0.01, yArray.max())
