...

{
    fun changeParams(name: String, illu:Double, temp:Double){
        ...
        this.Illu = illu
        this.temp = temp
        var startingTemp = Singleton.cellList.getCell(name).startingTemp
        var i_illu_coe = Singleton.cellList.getCell(name).iIlluCoe
        var i_temp_coe = Singleton.cellList.getCell(name).iTempCoe
        var v_temp_coe = Singleton.cellList.getCell(name).vTempCoe
        var startingIllu = Singleton.cellList.getCell(name).startingIllu
        this.xArray = Singleton.cellList.getCell(name).vArray
        this.yArray = Singleton.cellList.getCell(name).iArray
        var adjustedPair = MatrixCell.adjustForParams(xArray,yArray, startingTemp, v_temp_coe, i_temp_coe, temp, startingIllu, i_illu_coe, illu)
